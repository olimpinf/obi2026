...
from principal.models import LEVEL, Compet, CompetCfObi, School
...
def compute_points_compets(self,descriptor,level,school_id,compet_id):
    if compet_id != 0:
        compets = CompetCfObi.objects.filter(compet_type=level,compet_id=compet_id,compet__compet_school_id=school_id)
    elif school_id != 0:
        compets = CompetCfObi.objects.filter(compet_type=level,compet__compet_school_id=school_id).order_by('compet_id')
    else:
        compets = CompetCfObi.objects.filter(compet_type=level).order_by('compet_id')
    ...
    for c in compets:
        ...
        print(c.compet_id,c.compet_points)
        if c.compet_points==None:
            c.compet_points = total_points
            c.save()
            new += 1
            self.stdout.write(self.style.SUCCESS('******** NEW {} {}'.format(c.compet_id,c.compet_points)))
        elif c.compet_points < total_points:
            self.stdout.write(self.style.SUCCESS('******** UPDATE to greater {} {} --> {}'.format(c.compet_id,c.compet_points,total_points)))
            c.compet_points = total_points
            c.save()
            updated_to_greater += 1
        elif c.compet_points > total_points:
            self.stdout.write(self.style.SUCCESS('===== UPDATE to smaller {} {} --> {}'.format(c.compet_id,c.compet_points,total_points)))
            c.compet_points = total_points
            c.save()

...
MAXIMUM = {1: 20, 2: 20, 3: 300, 4: 400, 5: 300, 6: 400, 7: 20}
...
def compute_classif_one_state(level,state):
    ...
    compets = compets.filter(compet_points_fase2b__isnull=False).order_by('-compet_points_fase2b')
    ...
    min_points=100000
    for c in compets:
        if not c.compet_points_fase2b or c.compet_points_fase2b < MIN_POINTS:
            break
        if num_classif<=min_num_classif or min_points==c.compet_points_fase2b:
            min_points=c.compet_points_fase2b
            if not c.compet_classif_fase2b:
                c.compet_classif_fase2b = True
                #print("new {} {}".format(c.compet_id,c.compet_points_fase2b))
                c.save()
            num_classif += 1
...
def compute_classif_all_compets(self, level):
    ...
    compets_not_null = compets.filter(compet_points_fase2b__isnull=False)
    compets_classif = compets_not_null.filter(compet_classif_fase2b=True)
    compets_not_classif = compets_not_null.exclude(compet_classif_fase2b=True).order_by('-compet_points_fase2b')
    tot = len(compets)
    ...
    if target_num_classif <= num_classif:
        ...
        self.stdout.write(self.style.SUCCESS('min_points = {}'.format(compets_classif.order_by('compet_points_fase2b')[0].compet_points_fase2b)))
        return 
    ...
    for c in compets_not_classif:
        if c.compet_points_fase2b < MIN_POINTS:
            break
        if new_classif<target_num_new_classif or min_points==c.compet_points_fase2b:
            if min_points!=c.compet_points_fase2b: 
                print(min_points, new_classif)
            if new_classif<target_num_new_classif:
                min_points=c.compet_points_fase2b
            #self.stdout.write(self.style.SUCCESS('New classif id={}, points={}'.format(c.compet_id,c.compet_points_fase2b)))
            c.compet_classif_fase2b = True
            c.save()
            new_classif += 1
        elif new_classif>target_num_new_classif and min_points > c.compet_points_fase2b:
            print(min_points, new_classif)
            break
    sub_total = 0

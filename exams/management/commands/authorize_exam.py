...
def authorize_exam_school(exam, level,  school):
    ...
    compets = Compet.objects.filter(compet_school_id=school.school_id, compet_type=compet_type)
    count = 0
...
def authorize_exams(descriptor,level,school_id,compet_id):
    ...
    if school_id != 0:
        ...
        if compet_id != 0:
            ...
            compet_type = LEVEL[level.upper()]
            try:
                compet = Compet.objects.get(pk=compet_id,compet_type=compet_type,compet_school_id=school_id)
                count += authorize_exam_compet(exam,compet,school)
            except:
                print('no such compet in school',compet_id,school)
        else:
            # authorize all compets in school
            #if (level[0] == 'i' and school.school_turn_phase1_ini == 'B') or (level[0]=='p' and school.school_turn_phase1_prog == 'B'):
            count += authorize_exam_school(exam,level,school)
    else:
        schools = School.objects.all()
        # if level[0] == 'i':
        #     schools = School.objects.filter(school_turn_phase1_ini='B')
        # elif level[0] == 'p':
        #     schools = School.objects.filter(school_turn_phase1_prog='B')
        # else:
...
class Command(BaseCommand):
    help = 'Authorize exam in turn B, phase 1'

...
from exams.settings import EXAMS, TIMEZONE
...
def check_solutions_file(archive, level, exam_descriptor, school_id):
    ...
    table_submissions = EXAMS[exam_descriptor]['exam_table_submissions']
    cur_dir = os.getcwd()
    ...
    for root, dirs, files in os.walk(tmp_dir, topdown=False):
        ...
        for fname in files:
            ...
            if is_binary(sub_source):
                ...
                continue
            try:
                #submission = table_submissions.objects.get(compet = Compet(pk=compet_id),problem_name = problem_name)
                submission = table_submissions.objects.get(team_id = compet_id,problem_name = problem_name)
                submission.sub_source = sub_source
                submission.sub_lang = sub_lang
                action = 'Atualizado'
                print('existing submission',submission)
            except:
                submission = table_submissions(sub_source = sub_source,
                                  sub_lang = sub_lang,
                                  #compet = Compet(pk=compet_id),
                                  team_id = compet_id,
                                  problem_name = problem_name,
                                  problem_name_full = problem_name_full)
                print('new submission',submission)
                action = 'Inserido'
            submission.save().using(exam_db)
...
def usage():
    print("%s: inputarchive level exam_descriptor school" % (sys.argv[0]))
    os._exit(1)
...
if __name__ == "__main__":
    try:
        ...
        exam_descriptor =  sys.argv[3]
        school_id =  int(sys.argv[4])
    except:
        usage()
        os.exit(1)
            
    result = check_solutions_file(archive, level, exam_descriptor, school_id)
    for r in result:

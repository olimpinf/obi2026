...
if not MAC:
    ...
    BASE_CERTIFS="/Users/ranido/Documents/SBC/OBI/django3.2.nosync/obi2023/attic/certifs/"
    PDFLATEX = "/Library/TeX/texbin/pdflatex"
    #from medal_cuts import medal_cuts
    #from utils import slugfy
    from principal.utils.medal_cuts import medal_cuts
    from principal.utils.utils import slugfy
...
roman_year={2005:'VII',2006:'VIII',2007:'IX',2008:'X',2009:'XI',2010:'XII',2011:'XIII',2012:'XIV',2013:'XV',2014:'XVI',2015:'XVII',2016:'XVIII',2017:'XIX',2018:'XX',2019:'XXI',2020:'XXII',2021:'XXIII',2022:'XXIV',2023:'XXV'}
space_left={2005:0,2006:0,2007:28,2008:28,2009:28,2010:28,2011:14,2012:14,2013:14,2014:14,2015:14,2016:14,2017:14,2018:14,2019:14,2020:14,2021:14,2022:14,2023:14}
text_size={2005:'huge',2006:'huge',2007:'huge',2008:'huge',2009:'huge',2010:'huge',2011:'Huge',2012:'Huge',2013:'Huge',2014:'Huge',2015:'Huge',2016:'Huge',2017:'Huge',2018:'Huge',2019:'Huge',2020:'Huge',2021:'Huge',2022:'Huge',2023:'Huge'}

WEEK_DATE = {2019: 'no Instituto de Computação da Unicamp de 1 a 7 de dezembro de 2019', 2020: 'online de 6 a 10 de dezembro de 2021', 2021: 'online de 6 a 10 de dezembro de 2021', 2023: 'no Instituto de Computação da Unicamp de 4 a 10 de dezembro de 2023'}
...
def get_certif_school_compets(school_id,year,compet_type=0):
...
def get_week_certif_monitor(monitor_name, genre, hours, year):
    ...
    f.write(week_monitor_body_text % (space_left[year], 'LARGE', monitor_name, suffix, roman_year[year], year, hours))
    f.write(end_text)
    f.write(tail_text)
    f.close()
...
def get_week_certif_professor(professor_name, genre, course, year):
    ########
    #######
    # temporary: year is 2021 (Seletiva)
    year = 2022
    ...
    # build the tex file
    slug = slugfy(professor_name)
    f = open(os.path.join(BASE_CERTIFS,'CertifProfessor_%s_%d.tex' % (slug,year)), 'w')
    f.write(head_text % os.path.join(BASE_CERTIFS,f"FundoCertif{year}.pdf"))
    f.write(week_professor_body_text % (space_left[year], 'LARGE', professor_name, suffix, roman_year[year], year, course))
    f.write(end_text)
    f.write(tail_text)
    f.close()

    # build pdf
    build_pdf(os.path.join(BASE_CERTIFS,'CertifProfessor_%s_%d.tex' % (slug,year)))
    # return the certificate
    imgf=open(os.path.join(BASE_CERTIFS,'CertifProfessor_%s_%d.pdf' % (slug,year)),"rb")
    data=imgf.read()
    imgf.close()
    try:
      os.unlink(os.path.join(BASE_CERTIFS,'CertifProfessor_%s_%d.pdf' % (slug,year)))
      os.remove(os.path.join(BASE_CERTIFS,'CertifProfessor_%s_%d.tex' % (slug,year)))
      os.remove(os.path.join(BASE_CERTIFS,'CertifProfessor_%s_%d.log' % (slug,year)))
      os.remove(os.path.join(BASE_CERTIFS,'CertifProfessor_%s_%d.aux' % (slug,year)))
    except:
      pass
    return(data)
...
def get_certif_compet(compet_id,year):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        emit_certif_compet(compet_id,year,data,f)
        f.write(tail_text)
        f.close()
        # build pdf
        build_pdf('certif.tex')
...
def emit_certif_compet(compet_id,year,data,f,school_id=None):
    global gold,silver,bronze,honor
    if not data:
        return
    ...
    if compet_points_fase1!=None and compet_points_fase1>=0 and rank!=None:

...
def compute_points_compets(self,descriptor,level,school_id,compet_id):
    if compet_id != 0:
        compets = Compet.objects.filter(compet_type=level,compet_id=compet_id,compet_school_id=school_id,compet_classif_fase2=True)
    elif school_id != 0:
        compets = Compet.objects.filter(compet_type=level,compet_school_id=school_id,compet_classif_fase2=True).order_by('compet_id')
    else:
        compets = Compet.objects.filter(compet_type=level,compet_classif_fase2=True).order_by('compet_id')

    print(level,school_id,compet_id)
    ...
    for c in compets:
        ...
        for task in scores.keys():
            print('*****',task)
            p = _task_score_max_subtask(scores[task])
            print('  p', p)
            total_points += p
        ...
        if c.compet_points_fase3==None:
            c.compet_points_fase3 = total_points
            c.save()
            new += 1
            self.stdout.write(self.style.SUCCESS('******** NEW {} {}'.format(c.compet_id,c.compet_points_fase3)))
        elif c.compet_points_fase3 < total_points:
            self.stdout.write(self.style.SUCCESS('******** UPDATE to greater {} {} --> {}'.format(c.compet_id,c.compet_points_fase3,total_points)))
            c.compet_points_fase3 = total_points
            c.save()
            updated_to_greater += 1
        elif c.compet_points_fase3 > total_points:
            self.stdout.write(self.style.SUCCESS('===== UPDATE to smaller {} {} --> {}'.format(c.compet_id,c.compet_points_fase3,total_points)))
            c.compet_points_fase3 = total_points
            c.save()

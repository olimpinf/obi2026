...
LANG_GOOD_SUFIXES = ('c', 'cc', 'cpp', 'c++', 'pas', 'p', 'py', 'java', 'js')
LANG_SUFIXES = {"cc":2, "cpp":2, "c++":2, "c":1, "pas":3, "p":3, "py2":4, "java":5, "js":6, "py3":7}
PROBLEM_NAMES = {
5: {"pares":"Pares de Números","parcelamento":"Parcelamento sem juros", "manchas":"Manchas de pele"},
3: {"colecao":"Coleção de Upas", "linhas":"Linhas de ônibus", "parcelamento":"Parcelamento sem juros", "manchas":"Manchas de pele"},
4: {"metro":"Metrô da Nlogônia", "mesa":"Mesa redonda", "exploracao":"Exploração do Capitão Levi","prix":"Grand Prix da Nlogônia","etiquetas":"Etiquetas"},
6: {"xadrez":"Xadrez Aleatório", "metro":"Metrô da Nlogônia", "mesa":"Mesa redonda", "computador":"Computador", "etiquetas":"Etiquetas"}
}
...
def check_solutions_file(archive, level, phase, school_compets):
    ...
    for root, dirs, files in os.walk(tmp_dir, topdown=False):
        ...
        if compet_id_full != format_compet_id(compet_id):
            ...
            continue
        try:
            compet_ok = school_compets.get(compet_id=compet_id)
        except:
            compet_ok = ''
        if not compet_ok:
        ...
        for fname in files:
            ...
            if ext[1:] not in LANG_GOOD_SUFIXES:
                result.append(Error(compet_id_full,fname_orig,'Extensão do nome de  problema incorreta'))
                continue
            extension = ext[1:]
            if extension == "py":
                pat_name = re.compile("(.*)(_py2|_py3)")
                m = pat_name.match(problem_name)
                if not m:
                    result.append(Error(compet_id_full,fname_orig,'Nome de problema incorreto'))
                    continue
                problem_name = m.groups()[0]
                python_version = m.groups()[1]
                if python_version == '_py2':
                    extension = 'py2'
                elif python_version == '_py3':
                    extension = 'py3'
                else:
                    result.append(Error(compet_id_full,fname_orig,'Nome de problema incorreto'))
                    continue
            sub_lang = LANG_SUFIXES[extension]
            ...
            if is_binary(sub_source):
                ...
                continue
            try:
                submission = SubFase3.objects.get(compet = Compet(pk=compet_id),problem_name = problem_name)
                submission.sub_source = sub_source
                submission.sub_lang = sub_lang
                action = 'Atualizado'
            except:
                submission = SubFase3(sub_source = sub_source,
                                  sub_lang = sub_lang,

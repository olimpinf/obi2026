...
def build_binaries(directory):
    ...
    def build_binary(src_fn):
        ...
        if src_fn.endswith('.c'):
            cmd_line = '%s -O2 -std=c99 -o %s %s -lm -lncurses' % (gcc, exe_fn, src_fn)
    ...
    for src_fn in glob(os.path.join(directory, '*.py')):
        build_binary(src_fn)
    for src_fn in glob(os.path.join(directory, '*.js')):
        build_binary(src_fn)
...
def box_check_solutions(name, sol_groups, do_time=False, do_sample=False):
    ...
    def check_solution_language(fsolname):
        ...
        if os.path.exists(solname+'.py'):
            sols.append('python')
        if len(sols) > 1:
            show_message(name, "solutions with conflicting source name {}".format(fsolname), color = INFO)
        else:
            return sols[0]

    def run_check(name,sol_group,ctime_limit=100,jtime_limit=100,jstime_limit=100,ptime_limit=100):
    ...
    if "pass" in sol_groups:
        run_check(name,'pass',ct,jt,jst,pt)
        if short_mode: print
    if "wrong" in sol_groups:
        run_check(name,'wrong',ct,jt,jst,pt)
        if short_mode: print
    if "slow" in sol_groups:
        run_check(name,'slow',ct,jt,jst,pt)
        if short_mode: print
...
def timelimit(times):
    ...
    return max(0.01, 2*mintime, 2*maxtime)

...
class Entry(models.Model):
    ...
    class Meta:
        db_table = 'blog_restrito_entries'
    ...
    def get_absolute_url(self):
        ...
        return reverse('blog_restrito:entry', kwargs=kwargs)
    ...
    def invalidate_cached_entry(self):
        ...
        rf = RequestFactory(
            SERVER_NAME='olimpiada.ic.unicamp.br',
            SERVER_PORT=8000,
        ...
        try:
            ...
            #logger.info('cache.delete, settings.CACHE_MIDDLEWARE_ALIAS: {}'.format(settings.CACHE_MIDDLEWARE_ALIAS))
            #logger.info('cache.delete, settings.CACHE_MIDDLEWARE_KEY_PREFIX: {}'.format(settings.CACHE_MIDDLEWARE_KEY_PREFIX))
            #logger.info('cache.delete, url.path: {}'.format(url.path))
            #cache_key = get_cache_key(request, key_prefix=settings.CACHE_MIDDLEWARE_KEY_PREFIX)
            #logger.info('cache.delete, cache_key: {}'.format(cache_key))
            #if cache_key:

...
def authorize_exam_compet(exam,compet,school):
    if not compet.compet_classif_fase1:
...
def authorize_exam_school(exam, level,  school):
    ...
    compets = Compet.objects.filter(compet_school_id=school.school_id, compet_type=compet_type, compet_classif_fase1=True)
    count = 0
...
def authorize_exams(descriptor,level,school_id,compet_id):
    ...
    if school_id != 0:
        ...
        if compet_id != 0:
            ...
            compet_type = LEVEL[level.upper()]
            try:
                compet = Compet.objects.get(pk=compet_id,compet_type=compet_type,compet_school_id=school_id,compet_classif_fase1=True)
                count += authorize_exam_compet(exam,compet,school)
            except:

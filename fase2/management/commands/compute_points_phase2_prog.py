...
def compute_points_compets(self,descriptor,level,school_id,compet_id):
    print(f"in compute_points_compets level={level}, schooo_id={school_id}, compet_id={compet_id}")
    if compet_id != 0:
    ...
    for c in compets:
        ...
        if c.compet_points_fase2==None:
            ...
            self.stdout.write(self.style.SUCCESS('******** UPDATE to greater {} {} --> {}'.format(c.compet_id,c.compet_points_fase2,total_points)))
            c.compet_points_fase2 = total_points
            c.save()
            updated_to_greater += 1
        elif c.compet_points_fase2 > total_points:
            self.stdout.write(self.style.SUCCESS('===== UPDATE to smaller {} {} --> {}'.format(c.compet_id,c.compet_points_fase2,total_points)))
            c.compet_points_fase2 = total_points
            c.save()
            ...
            not_updated += 1
    print("new:", new)
    print("updated_to_smaller:", updated_to_smaller)
    print("updated_to_greater:", updated_to_greater)

...
from exams.models  import ExamFase1b
...
def authorize(self,c,exam):
    ...
    try:
        ...
        self.stdout.write(self.style.SUCCESS('Autorização já existe para {}.'.format(c)))
        return 0
    except:
        # not authorized, then authorize, but only if compet has email
        if c.user:
            ex = exam(compet=c,school=school)
            ex.save()
            self.stdout.write(self.style.SUCCESS('Autorização OK para {}.'.format(c)))
            return 1
...
def fix_compet_auth(self):
    ...
    if PROG:
        ...
        levels = (1,2,7)
        compets = Compet.objects.filter(compet_type__in=levels, compet_school__school_turn_phase1_ini='B')
        schools = School.objects.filter(school_turn_phase1_ini='B').only('school_id','school_name')
    #print('compets',len(compets))
    authorized = ExamFase1b.objects.all().only('compet_id')
    #print('authorized',len(authorized))
    count = 0
    print(compets.filter(compet_id=20259))
    tmp = Compet.objects.get(compet_id=20259)
    print(tmp)
    tmp1 = School.objects.get(school_id=tmp.compet_school_id)
    print(tmp1)
    print(tmp1.school_turn_phase1_ini)
    for c in compets:
        if c.compet_id==20259:
            print(c)
        if not authorized.filter(compet_id=c.compet_id).exists():
            count += authorize(self,c,ExamFase1b)
    return count

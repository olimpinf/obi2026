...
def send_queued_messages():
    ...
    # to test messages
    #messages = QueuedMail.objects.filter(to_addr__icontains='hotmail.com',id=18908).order_by("id")

    messages = QueuedMail.objects.filter(to_addr__icontains='hotmail.com',id__gt=LAST_MESSAGE_ID,sent=False).order_by("id")
    for m in messages:
        if not DO_SEND:
            continue

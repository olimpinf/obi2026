...
def fix_compet_auth(self):
    ...
    if PROG:
        ...
        compets = Compet.objects.filter(compet_type__in=levels)
        #schools = School.objects.filter(school_turn_phase1_prog='B').only('school_id','school_name')
        schools = School.objects.all().only('school_id','school_name')
    else:
        levels = (1,2,7)

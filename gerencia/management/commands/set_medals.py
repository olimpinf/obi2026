...
def set_medals(self,year,compet_type):
    ...
    compets_all = Compet.objects.filter(compet_points_fase1__gte=0).order_by('compet_rank_final')
    for compet_type in compet_types:
        nmedals = {'o': 0, 'p': 0, 'b': 0, 'h': 0, 'x':0}
        gold,silver,bronze,honor,mod,level,total=medal_cuts(compet_type,year)

    num_gold,num_silver,num_bronze,num_honor = 0,0,0,0
    compets = compets_all.filter(compet_type=compet_type)
    for c in compets:
        rank = c.compet_rank_final
        if not rank:
            continue
        if (rank<=gold):
            medal='o'
        ...
        if medal != 'h':
            school = schools_all.get(school_id=c.compet_school_id)

...
for line in fin2.readlines(): #[2:]:
    ...
    cities_coord_all[city]=[line[2].strip(),line[3].strip()]
...
for line in fin.readlines(): #[2:]:
    try:
        ...
        if city not in cities.keys():
            cities[city]={"coords":cities_coord_all[city]}
            cities_coord[city]=cities_coord_all[city]
        cities[city][line[2].strip()]=line[3].strip()
        cities[city]["escolas"]=line[4].strip()
    except:
...
for c in cities.keys():
    ...
    for level in ["I1","I2","IJ","PJ","P1","P2","PS"]:
